#This is modified Code taken from: https://www.tensorflow.org/addons/api_docs/python/tfa/layers/esn
...
class ESNCell(keras.layers.AbstractRNNCell):
    ...
    def build(self, inputs_shape):
        ...
        def _esn_kernel_initializer(shape, dtype, partition_info=None):
            
            ( num_inputs, nodes ) = shape

            q = nodes // num_inputs
            initializer = tf.keras.initializers.RandomUniform( minval = -self.sigma, maxval = self.sigma )
            w_in = tf.zeros( ( 0, nodes ), dtype )
            for i in range( num_inputs ):

                w_in_i = initializer( ( 1, q ) )
                w_in_i = tf.concat( [ tf.zeros( shape =( 1, i * q ) ), w_in_i, tf.zeros( shape = ( 1, nodes - ( i + 1 ) * q ) ) ], 1 )
                w_in = tf.concat( [ w_in,  w_in_i ], 0 )

            return w_in

        def _esn_recurrent_initializer(shape, dtype, partition_info=None):
            recurrent_weights = tf.keras.initializers.RandomUniform( 0, 1 )(
                shape, dtype
            )
            connectivity_mask = tf.cast(
                tf.math.less_equal( tf.random.uniform(shape), self.connectivity,), dtype
            )
            recurrent_weights = tf.math.multiply( recurrent_weights, connectivity_mask) #Absolute values like in paper
            # Satisfy the necessary condition for the echo state property `max(eig(W)) < 1`
            if self.use_norm2:
                # This condition is approximated scaling the norm 2 of the reservoir matrix
                # which is an upper bound of the spectral radius.
                ...
                #abs_eig_values = tf.abs(tf.linalg.eig(recurrent_weights)[0])
                abs_eig_values = tf.abs( np.linalg.eig(recurrent_weights)[0] )#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! A lot faster on CPU than tf. !
                scaling_factor = tf.math.divide_no_nan(
                    self.spectral_radius, tf.reduce_max(abs_eig_values)
                )
        ...
        self.kernel = self.add_weight(
            ...
            initializer=_esn_kernel_initializer,
            trainable=False,
            dtype=self.dtype,
        )
        if( self.sparse ):
            self.kernel = tf.sparse.from_dense( self.kernel )
            self.recurrent_kernel = tf.sparse.from_dense( self.recurrent_kernel )
    ...
    def call(self, inputs, state):
        if( self.sparse ):
            output = tf.transpose( tf.sparse.sparse_dense_matmul( self.kernel, inputs, adjoint_a = True, adjoint_b = True ) )
            output += tf.transpose( tf.sparse.sparse_dense_matmul( self.recurrent_kernel, state[0], adjoint_a = True, adjoint_b = True ) )
        else:
